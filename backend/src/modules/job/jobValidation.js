...
export const createJobValidation = [
  ...
  body("salary.min")
    ...
    .isInt({ min: 0 }).withMessage("Minimum salary must be >= 0"),

  body("salary.max")
    .notEmpty().withMessage("Maximum salary is required")
    .isInt({ min: 0 }).withMessage("Maximum salary must be >= 0")
    .custom((value, { req }) => value >= req.body.salary.min)
    .withMessage("Maximum salary must be greater than or equal to minimum salary"),

  body("experience.min")
    .notEmpty().withMessage("Minimum experience is required")
    .isInt({ min: 0 }).withMessage("Minimum experience must be in months"),

  body("experience.max")
    .notEmpty().withMessage("Maximum experience is required")
    .isInt({ min: 0 }).withMessage("Maximum experience must be in months")
