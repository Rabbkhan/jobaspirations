...
const BlogDetailsPage = () => {
    ...
    const fetchRelatedBlogs = async () => {
        try {
            const { data } = await axios.get(`${PUBLIC_BLOG_API_END_POINT}/related/${slug}`)
    ...
    const fetchLatestBlogs = async () => {
        try {
            const { data } = await axios.get(`${PUBLIC_BLOG_API_END_POINT}/latest/?page=1&limit=4`)
        ...
        <div className="bg-background py-12">
            <div className="container mx-auto px-4 grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <span className="inline-block rounded-full bg-primary/10 px-4 py-1 text-sm font-medium text-primary">{blog.category?.name}</span>

                    <h1 className="text-4xl font-bold tracking-tight leading-tight text-foreground">{blog.title}</h1>

                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                        <span className="font-medium text-foreground">{blog.author?.fullname}</span>
                        <span>•</span>
                        <span>
  ...
  "
                    ...
                    <div className="mt-12 border-t pt-6 flex items-center gap-4">
                        ...
                        <div>
                            <p className="font-semibold text-foreground">{blog.author?.fullname}</p>
                            <p className="text-sm text-muted-foreground">Administrator & Content Author</p>
                        </div>
                    </div>
